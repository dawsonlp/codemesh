# Intent & Requirements Authority: Requirements & Specification

> **Domain Role**: The single source of truth for **Intent Semantics** (why the software exists, what business goals it satisfies, which architectural decisions govern it, and what constraints are enforced).

---

## 1. Mission & Domain Boundary

The **Intent Authority** captures human and organizational purpose. It bridges abstract business requirements, architectural decision records (ADRs), regulatory mandates, and engineering guardrails into structured, machine-navigable entities.

### Authority Separation:
* **Intent Authority Owns**: Functional requirements, user stories, architectural decisions (ADRs), non-functional quality attributes (SLAs/SLOs), governance policies (security, compliance, privacy), and executable constraint definitions.
* **CodeMesh Owns**: Computational implementation (classes, functions, methods, call graphs, tests).
* **Data Authority Owns**: Conceptual, logical, and physical data structures.
...
The Intent Authority exposes deterministic, versioned URIs across five canonical entity types:

### A. Requirement URIs (`req://`)
Format: `req://<domain>/<id-or-slug>`
* Example: `req://payments/idempotent-charge-execution`
* Example: `req://auth/multi-factor-mandatory-for-admins`

### B. Architectural Decision URIs (`decision://`)
Format: `decision://<domain>/<adr-number>-<slug>`
* Example: `decision://payments/adr-004-stripe-idempotency-keys`
* Example: `decision://messaging/adr-009-event-driven-order-fulfillment`

### C. Constraint URIs (`constraint://`)
Format: `constraint://<domain>/<id-or-slug>`
* Example: `constraint://architecture/domain-services-must-not-import-db-drivers`
* Example: `constraint://orders/order-total-must-match-item-sum`

### D. Policy URIs (`policy://`)
Format: `policy://<domain>/<id-or-slug>`
* Example: `policy://security/no-raw-sql-in-controllers`
* Example: `policy://privacy/gdpr-right-to-erasure-compliance`

### E. Quality Attribute URIs (`quality://`)
Format: `quality://<domain>/<id-or-slug>`
* Example: `quality://checkout/p99-latency-under-200ms`
* Example: `quality://availability/99-99-uptime-during-peak`
...
The Intent Authority must maintain typed relationships across intent nodes and incoming edges from CodeMesh and Data domains:

| Relational Verb | Source | Target | Meaning |
| :--- | :--- | :--- | :--- |
...
### FR-2: Executable vs. Declarative Constraints
The Intent Authority must support a dual-representation model for constraints:

1. **Declarative Intent (For LLM Context)**:
...
* **`DERIVED`**: Extracted deterministically from structured specifications, OpenAPI annotations, or verified test assertions.
...
* When an ADR transitions to `SUPERSEDED`, the Intent Authority alerts CodeMesh to identify all code symbols linked via `GOVERNED_BY` to schedule refactoring.

### FR-5: Actionable Diagnostic Engine with Remediation Hints
When an executable constraint is violated during `MutationEngine.validate_invariants()`, the Intent Authority must return structured diagnostics:
```json
{
...
The Intent Authority must sync bi-directionally with:
* **Architectural Decision Records**: Markdown files in `docs/adr/*.md` following MADR or Nygard templates.
* **Issue Trackers & PRDs**: Jira, Linear, GitHub Issues, Notion, PRD markdown files.
* **Policy Engines**: Open Policy Agent (OPA), AWS IAM / Cedar policies, HashiCorp Sentinel.

---

## 4. Query & Resolution API Specification

The Intent Authority must expose fast ($< 50\text{ms}$) query APIs:
...
Executes all registered executable constraint validators against the proposed ASTs or graph mutations in memory.
...
└───────────────────────────────────┬────────────────────────────────────┘
                                    ...
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 1. SLICE CONTEXT: CodeMesh pulls target code + relevant ADRs & Policies│
│    (ContextSlicer combines .pyi contracts + req:// summaries)          │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 2. PROPOSE EDIT: Agent emits zero-diff symbol edit (edit_symbol)       │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 3. INVARIANT GATE: Intent Authority executes active constraints        │
│    (Checks architecture boundaries, state rules, security policies)   │
│    • Passed -> Proceed to disk projection                              │
│    • Failed -> Returns structured remediation hint to Agent            │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ 4. PERSISTENCE: CodeMesh projects code to disk; Intent graph records   │
